...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(Path.home() / 'context-sync' / 'logs' / f'sync_{datetime.now().strftime("%Y%m%d")}.log')
    ]
)
...
def sync_all_connectors(incremental: bool = True) -> Dict:
    ...
    for connector in connectors:
        ...
        try:
            ...
            results['connectors'][connector.name] = {
                'success': success,
                'metadata': metadata
            }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Sync all context-sync connectors'
    )
    parser.add_argument(
        '--full',
        action='store_true',
