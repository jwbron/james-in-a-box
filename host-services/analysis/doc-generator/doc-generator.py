#!/usr/bin/env python3
"""
LLM Documentation Generator - Multi-Agent Pipeline
...
class DocumentationGenerator:
    """Multi-agent documentation generation pipeline."""
    ...
    def generate_draft(self, context: DocContext, doc_type: str) -> DraftDoc:
        ...
        if doc_type == "status-quo":
            ...
            return self._draft_best_practice(context)
    ...
    def _draft_status_quo(self, context: DocContext) -> DraftDoc:
        ...
        if context.components:
            ...
            sections.append("|-----------|------|----------|-------------|")
            for comp in context.components[:10]:  # Limit to 10
        ...
        if context.code_examples:
            ...
            sections.append("")
            for example in context.code_examples[:10]:  # Limit to 10
        ...
        if context.existing_docs:
            ...
            sections.append("")
            for doc in context.existing_docs[:5]:  # Limit to 5
    ...
    def _draft_best_practice(self, context: DocContext) -> DraftDoc:
        """Generate best practice documentation with external validation hints."""
        ...
        sections.append(
            f"> Auto-generated by jib on {datetime.now(timezone.utc).strftime('%Y-%m-%d')}. "
            "Includes external research recommendations."
        )
        sections.append("")
        ...
        sections.append("")
        sections.append(
            f"*TODO: Research current best practices for {context.topic}. "
            "Check official documentation, OWASP guidelines, and authoritative sources.*"
        )
        sections.append("")
        ...
        sections.append("")
        sections.append("| Practice | Status | Notes |")
        sections.append("|----------|--------|-------|")
        sections.append("| *Best practice 1* | *TBD* | *Research needed* |")
        sections.append("| *Best practice 2* | *TBD* | *Research needed* |")
        sections.append("")

        # Recommendations placeholder
        sections.append("## Recommendations")
        sections.append("")
        sections.append("*Recommendations will be added after external research.*")
        sections.append("")

        # Sources section
        sections.append("## Sources")
        sections.append("")
        sections.append("*Add authoritative sources after research:*")
        sections.append("- Official documentation")
        sections.append("- RFCs and standards")
        sections.append("- Security guidelines (OWASP, NIST)")
        sections.append("")
        ...
        return DraftDoc(
            ...
            sources=["patterns.json", "external_research_needed"],
            warnings=["External research needed to complete this document"],
        )
    ...
    def review_draft(self, draft: DraftDoc, context: DocContext) -> ReviewResult:
        ...
        for example in context.code_examples[:3]:  # Spot check first 3
            if ":" in example:
    ...
    def save_documentation(
        ...
        # Add review notes if there were issues
    ...
    # =========================================================================
    def generate(
        self, topic: str, doc_type: str = "status-quo", dry_run: bool = False
    ) -> dict[str, Any]:
        ...
        if dry_run:
            ...
            print(draft.content[:1000])
            if len(draft.content) > 1000:
                print(f"\n... ({len(draft.content) - 1000} more characters)")
        else:
    ...
    def generate_all(self, doc_type: str = "status-quo", dry_run: bool = False) -> list[dict]:
        """Generate documentation for all detected patterns."""
        results = []
        topics = self.list_available_topics()

        # Only generate for topics with detected patterns
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate documentation using multi-agent pipeline",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  %(prog)s --list-topics                    # List available topics
  %(prog)s --topic auth --type status-quo   # Generate auth status quo docs
  %(prog)s --topic security --type best-practice  # Best practice docs
