...
class MinimalSetup:
    ...
    def _write_default_network_env(self) -> None:
        """Write default network.env for gateway (public mode).

        Private mode is determined by CLI flags (jib --private), not by setup.
        This just ensures the gateway has a default mode to start with.
        """
        network_env_file = self.config_manager.config_dir / "network.env"
        # Only write if file doesn't exist (preserve existing mode)
        if network_env_file.exists():
            return

        # Default to public mode
        lines = [
            "# Auto-generated by setup.py",
            "# Mode is controlled by CLI flags: jib --private or jib --public",
            "PRIVATE_MODE=false",
        ]
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            # Write default network.env for gateway (if not already present)
            self._write_default_network_env()
