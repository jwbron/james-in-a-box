# ADR: Interactive Planning Framework for Autonomous Development
...
- [The Interactive Planning Framework](#the-interactive-planning-framework)
...
This ADR establishes a **comprehensive interactive planning framework** that enables jib to:
...
**We will implement an Interactive Planning Framework (IPF) that uses a multi-agent architecture to guide users from vague intent through comprehensive planning to autonomous implementation.**
...
The Interactive Planning Framework (IPF) is a structured, multi-phase process that transforms vague user intent into comprehensive, implementation-ready documentation through guided human-AI collaboration.
...
---
*Generated by IPF Phase 1 | Approved: [Date]*
```
...
---
*Generated by IPF Phase 2 | Approved: [Date]*
```
...
---
*Generated by IPF Phase 3 | Approved: [Date]*
```
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    IPF IMPLEMENTATION ARCHITECTURE                            │
│                                                                               │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
...
**Tasks:**
1. Implement IPF Orchestrator with state machine
...
| **IPF MVP** (4 agents) | ~64,000 | ~$1.15 | Comprehensive planning, fewer implementation revisions |
| **IPF Extended** (5 agents + Review) | ~85,000 | ~$1.53 | Adds cross-validation |
| **IPF Full** (7 agents) | ~120,000 | ~$2.16 | Full specialist coverage |

### Break-Even Analysis

IPF is economically justified when:

1. **Implementation revisions avoided:** If IPF saves even 1 implementation revision cycle (~30,000 tokens), it breaks even
...
### When to Use IPF vs. Simpler Approaches

| Task Complexity | Approach | Rationale |
|-----------------|----------|-----------|
| **Simple** (bug fix, small feature) | Single-agent reactive | Low overhead, clear requirements |
| **Medium** (new feature, integration) | IPF with 2 checkpoints | Balance between planning and speed |
| **Complex** (new system, architecture change) | Full IPF | Comprehensive planning prevents costly rework |
...
To address checkpoint fatigue risk, IPF supports flexible checkpoint configurations.
...
|-----|--------------|---------------------|
| [ADR-Multi-Agent-Pipeline-Architecture](ADR-Multi-Agent-Pipeline-Architecture.md) | Provides reusable agent infrastructure (orchestration, dispatch, state management) that IPF specializes for planning workflows. IPF is a *consumer* of Multi-Agent Pipeline, not a dependency. | Multi-Agent Pipeline FIRST, then IPF |
...
The Multi-Agent Pipeline Architecture ADR and IPF are complementary but distinct:
...
- **IPF** is a *specific application* of that infrastructure for planning workflows. It defines the 4-phase planning process, specialized planning agents (Elicitation, Architecture, Planning, Review), and human checkpoint patterns.
...
2. **IPF** is then implemented as a workflow that runs on that infrastructure
3. IPF's "Orchestrator Agent" is an instance of the Multi-Agent Pipeline's AgentOrchestrator configured for planning

This is analogous to: Multi-Agent Pipeline is like Kubernetes; IPF is like a specific application deployed on Kubernetes.
