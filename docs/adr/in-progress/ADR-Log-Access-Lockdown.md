...
| `/api/v1/logs/search` | GET | `?pattern=<regex>&scope=self` | Matching log entries | Search limited to own logs (see [Regex Safety](#regex-safety)) |
| `/api/v1/logs/model/<task_id>` | GET | - | Full model output | Task must belong to requester |

#### Regex Safety

The `/api/v1/logs/search` endpoint accepts regex patterns, which introduces potential denial-of-service risks via ReDoS (Regular Expression Denial of Service) attacks.

**Mitigations:**

1. **Execution timeout**: Regex execution is limited to 5 seconds per search. Searches exceeding this limit return HTTP 408 (Request Timeout) with an error message explaining the pattern was too complex.

2. **Pattern complexity validation**: Before execution, patterns are analyzed for known problematic constructs:
   - Reject patterns with excessive backtracking potential (e.g., `(a+)+$`)
   - Reject patterns exceeding 500 characters
   - Reject patterns with more than 10 nested groups

3. **Result limits**: Search returns at most 1000 matches per request, reducing the impact of expensive patterns.

4. **Audit logging**: Search patterns are logged in the audit trail for forensic analysis. Pattern: `{"event_type": "log_search", "pattern": "<user-pattern>", ...}`
...
def safe_search(pattern: str, content: str, timeout_seconds: int = 5) -> list[str]:
    """Execute regex search with timeout and validation."""
    # Validate pattern complexity
    if len(pattern) > 500:
        raise ValueError("Pattern too long (max 500 chars)")
    if pattern.count("(") > 10:
        raise ValueError("Too many groups (max 10)")
    ...
    try:
        ...
        matches = regex.findall(content)[:1000]  # Limit results
        return matches
    finally:
...
When multiple orchestrator operations occur simultaneously (e.g., container creation, termination, or index updates), the following behaviors are expected:

**Atomicity Guarantee:** File locking (`fcntl.LOCK_EX`) ensures that index writes are serialized. Concurrent writers will block until the lock is released.

**Race Condition Scenarios:**

| Scenario | Expected Behavior |
|----------|-------------------|
| Container A terminates while Container B starts | Index updates are serialized; no data loss |
| Gateway reads while orchestrator writes | Gateway sees either pre-write or post-write state (no partial reads) |
| Two containers request same task's logs simultaneously | Both requests use gateway's in-memory cache; consistent results |
| Orchestrator crashes mid-update | Lock is released on process exit; next writer acquires lock normally |

**Task ID Uniqueness:** Task IDs include timestamps (`task-YYYYMMDD-HHMMSS`) and are generated by the orchestrator. The orchestrator ensures uniqueness by including milliseconds or a random suffix when needed. Two containers cannot be assigned the same `task_id`.

**Gateway Cache Consistency:** The gateway's in-memory cache of the log index is updated via `inotify` file watches. During the brief window between index file write and `inotify` notification delivery (~1-10ms), the gateway may serve slightly stale data. This is acceptable because:
1. The stale data is still valid (previous snapshot)
2. Policy decisions remain correct (container ownership doesn't change retroactively)
3. The window is too short to exploit
...
Since all log access routes through the gateway, gateway availability becomes critical for log access. This section documents the impact and mitigations.

**Failure Scenarios:**

| Scenario | Container Impact | Mitigation |
|----------|-----------------|------------|
| Gateway down | Log access fails with connection error | Container retries with exponential backoff; logs still being written to disk |
| Gateway restart | Brief interruption (~5s) | Client retry logic handles transient failures |
| Gateway overloaded | Slow or failed requests | Rate limiting prevents cascade; health checks detect issues |
| Network partition | Log access fails | Same as "gateway down"; no workaround from container |

**Design Decisions:**

1. **Fail-closed is correct**: If the gateway is unavailable, containers cannot access logs. This is the intended behavior—security over availability.

2. **Log writing is unaffected**: The gateway mediates **read** access only. Log writing continues normally via `container_logging.py` regardless of gateway state. Logs are not lost.

3. **Gateway is single point of failure for log reads**: This is acceptable because:
   - The gateway is already critical infrastructure (git access depends on it)
   - A single gateway serves all containers (no per-container gateway)
   - Gateway failures are rare and brief (container orchestration handles restarts)

**Operational Requirements:**

If log access is operationally critical (e.g., for automated debugging or health checks), the following are recommended:

| Requirement | Implementation |
|-------------|----------------|
| Health monitoring | `GET /api/v1/health` returns 200 when gateway can access log directories |
| Alerting | Alert on gateway unavailability > 30 seconds |
| Restart policy | `restart: unless-stopped` in Docker Compose for gateway container |
| Graceful degradation | Containers should not crash if log access fails; handle errors gracefully |

**Note:** For debugging during gateway outages, administrators can read logs directly from the host filesystem (`~/.jib-sharing/`). This bypasses security controls but is acceptable for emergency troubleshooting by humans with host access.
