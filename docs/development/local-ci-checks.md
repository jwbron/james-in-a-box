...
### Full GitHub Actions with act (Host Only)

If you're on the host machine with Docker available, you can run the exact GitHub Actions:
...
make act-lint    # Lint workflow
make act-test    # Test workflow
```
...
1. **`make check`** - Lightweight checks that work inside the jib container
   - No Docker required
   - Installs tools via pip if needed
   - Covers ~90% of CI failures

2. **`make act`** - Full GitHub Actions simulation
   - Requires Docker (runs on host)
   - Exact replica of CI environment
   - Useful for debugging complex CI issues
