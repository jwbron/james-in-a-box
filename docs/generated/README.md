...
**Note:** The JSON index files are **gitignored** and generated on container startup. This avoids merge conflicts caused by volatile data (timestamps, line numbers that shift with code changes).
...
Indexes are **automatically regenerated on container startup** to ensure they're always current.

Regenerate indexes manually (if needed):
...
Enable weekly auto-regeneration on host (optional, for non-container use):
...
|-------|-----------|---------|
| All indexes | Container startup | Automatic (entrypoint.sh) |
| All indexes | On-demand | `bin/generate-index` |
| All indexes | Weekly (Sunday 3am) | Systemd timer (if enabled on host) |
