...
class TestDiscovery:
    ...
    def _check_javascript_tests(self):
        ...
        # Check for Jest
        jest_config_files = ['jest.config.js', 'jest.config.ts', 'jest.config.json', 'jest.config.mjs']
        ...
        if jest_found:
            config_file = next((f for f in jest_config_files if (self.project_root / f).exists()), None)
            ...
            framework = TestFramework(
                ...
                watch_command=f"{test_cmd} -- --watch" if test_cmd == 'npm test' else 'jest --watch',
                coverage_command=f"{test_cmd} -- --coverage" if test_cmd == 'npm test' else 'jest --coverage',
            )
            self.result.frameworks.append(framework)
        ...
        # Check for Vitest
            ...
            framework = TestFramework(
                name='vitest',
                language='javascript',
                config_file='vitest.config.ts' if (self.project_root / 'vitest.config.ts').exists() else None,
        ...
        playwright_config = next((f for f in playwright_config_files if (self.project_root / f).exists()), None)
    ...
    def _check_java_tests(self):
        ...
        if (self.project_root / 'build.gradle').exists() or (self.project_root / 'build.gradle.kts').exists():
            config_file = 'build.gradle' if (self.project_root / 'build.gradle').exists() else 'build.gradle.kts'
            framework = TestFramework(
                name='gradle',
                language='java',
                config_file=config_file,
        ...
        for scripts_dir in scripts_dirs:
            ...
            for pattern in test_script_patterns:
                for script in dir_path.glob(pattern):
                    self.result.notes.append(f"Test script found: {script.relative_to(self.project_root)}")
        ...
        for framework in self.result.frameworks:
            for pattern in framework.test_patterns:
                for test_file in self.project_root.rglob(pattern):
                    ...
                    if any(part in test_file.parts for part in ['node_modules', 'vendor', '.git', 'dist', 'build']):
                        continue
    ...
    def run_tests(self, files: Optional[List[str]] = None) -> int:
...
def main():
    ...
    parser.add_argument('--files', help='Comma-separated list of changed files (for targeted testing)')
