...
class PRCreator:
    ...
    def create_notification(self, pr_result: Dict[str, Any], task_context: str = ""):
        """Create notification about PR creation"""
        timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        notif_file = self.notifications_dir / f"{timestamp}-pr-created.md"

        with notif_file.open('w') as f:
            if pr_result["success"]:
                f.write(f"# Pull Request Created\n\n")
                f.write(f"**URL**: {pr_result['url']}\n")
                f.write(f"**Branch**: `{pr_result['branch']}` -> `{pr_result['base']}`\n")
                if pr_result.get('title'):
                    f.write(f"**Title**: {pr_result['title']}\n")
                if pr_result.get('reviewer'):
                    f.write(f"**Reviewer**: @{pr_result['reviewer']}\n")
                if pr_result.get('already_existed'):
                    f.write(f"\n*Note: PR already existed for this branch*\n")
                if task_context:
                    f.write(f"\n## Context\n\n{task_context}\n")
                f.write(f"\n---\nGenerated by jib PR helper\n")
            else:
                f.write(f"# PR Creation Failed\n\n")
                f.write(f"**Error**: {pr_result.get('error', 'Unknown error')}\n")
                if task_context:
                    f.write(f"\n## Context\n\n{task_context}\n")
                f.write(f"\n---\nGenerated by jib PR helper\n")

        print(f"Notification created: {notif_file.name}")
        return notif_file
