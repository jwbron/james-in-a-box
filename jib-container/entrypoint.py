...
class Config:
    ...
    gemini_model: Optional[str] = field(default_factory=lambda: os.environ.get("GEMINI_MODEL"))
    
    # GitHub tokens
    github_token: Optional[str] = field(default_factory=lambda: os.environ.get("GITHUB_TOKEN"))
    github_readonly_token: Optional[str] = field(default_factory=lambda: os.environ.get("GITHUB_READONLY_TOKEN"))
...
def run_cmd(
    ...
    capture: bool = False,
    timeout: int = 30,
    as_user: Optional[tuple[int, int]] = None,
...
def setup_claude(config: Config, logger: Logger) -> None:
    ...
    trace_collector = config.khan_dir / "james-in-a-box/host-services/analysis/trace-collector/hook_handler.py"
    ...
    settings = {
        ...
        "hooks": {
            "PostToolUse": [{
                ...
                "hooks": [{
                    "type": "command",
                    "command": f"python3 {trace_collector} post-tool-use"
                }]
            }],
            "SessionEnd": [{
                "hooks": [
    ...
    chown_recursive(config.user_home / ".config/claude-code", config.runtime_uid, config.runtime_gid)
...
def setup_gemini(config: Config, logger: Logger) -> None:
    ...
    if config.google_api_key:
        ...
        env_file.write_text(f"# Auto-configured by jib entrypoint\nGEMINI_API_KEY={config.google_api_key}\n")
...
def setup_router(config: Config, logger: Logger) -> None:
    ...
    if config.llm_provider in ("google", "gemini"):
        ...
        logger.info("  Router logs: ~/.claude-code-router/logs/ and ~/.claude-code-router/claude-code-router.log")
...
def start_router(config: Config, logger: Logger) -> Optional[int]:
    ...
    with open(router_log, "w") as log_file:
        proc = subprocess.Popen(
            ["gosu", f"{config.runtime_uid}:{config.runtime_gid}", "ccr", "start", "--port", "3456"],
            stdout=log_file,
            stderr=log_file,
        )
    ...
    try:
        ...
        logger.info(f"  ANTHROPIC_AUTH_TOKEN={'set' if os.environ.get('ANTHROPIC_AUTH_TOKEN') else 'not set'}")
...
def generate_docs_indexes(config: Config, logger: Logger) -> None:
    ...
    index_generator = config.khan_dir / "james-in-a-box/host-services/analysis/index-generator/index-generator.py"
    ...
    try:
        run_cmd(
            ["python3", str(index_generator),
             "--project", str(jib_dir),
             "--output", str(jib_dir / "docs/generated")],
            as_user=(config.runtime_uid, config.runtime_gid),
            capture=True,
        )
...
def run_interactive(config: Config, logger: Logger) -> None:
    ...
    os.execvpe(
        "gosu",
        ["gosu", f"{config.runtime_uid}:{config.runtime_gid}",
...
def run_exec(config: Config, logger: Logger, args: list[str]) -> None:
    ...
    if config.llm_provider in ("google", "gemini"):
        ...
        if config.router_config.exists():
