...
# Add jib runtime scripts to PATH (copied to /opt/jib-runtime during build)
# bin/ contains symlinks to all executables for easy access
export PATH="/opt/jib-runtime/jib-container/bin:/usr/local/bin:${PATH}"
...
# Set BEADS_DIR so bd can be run from any directory (no need to cd ~/beads first)
export BEADS_DIR="${USER_HOME}/beads/.beads"
...
if [ -n "${GITHUB_TOKEN:-}" ]; then
    gosu "${RUNTIME_UID}:${RUNTIME_GID}" git config --global credential.helper "/opt/jib-runtime/jib-container/bin/git-credential-github-token"
    # Enable path-based credentials so the helper can distinguish between repo owners
    # (e.g., Khan/webapp vs jwbron/james-in-a-box for token selection)
    gosu "${RUNTIME_UID}:${RUNTIME_GID}" git config --global credential.useHttpPath true
...
if [ -d "${USER_HOME}/.git-main" ]; then
    for repo_config in "${USER_HOME}/.git-main"/*/config; do
        if [ -f "$repo_config" ]; then
            ...
            # Convert SSH URLs to HTTPS (container has no SSH keys, only GITHUB_TOKEN)
            # SSH format: git@github.com:owner/repo.git
            # HTTPS format: https://github.com/owner/repo.git
            if [[ "$remote_url" =~ ^git@github\.com: ]]; then
                https_url=$(echo "$remote_url" | sed -E 's|^git@github\.com:|https://github.com/|')
                git config -f "$repo_config" remote.origin.url "$https_url"
                log_info "  Converted SSH to HTTPS: $https_url"
            fi
        fi
    done
fi
...
if [ -d "${USER_HOME}/sharing" ]; then
    ...
    log_info "  Convenience symlink: ~/tmp → ~/sharing/tmp"
else
...
# Order: mission (core) → environment (constraints) → beads (task tracking) → context → standards → quality
...
if [ -f "${RULES_DIR}/mission.md" ]; then
    ...
    # Also create symlink in ~/khan/ where claude starts
    # Claude looks for CLAUDE.md in the current working directory
    if [ -d "${USER_HOME}/khan" ]; then
        ln -sf "${USER_HOME}/CLAUDE.md" "${USER_HOME}/khan/CLAUDE.md"
        chown -h "${RUNTIME_UID}:${RUNTIME_GID}" "${USER_HOME}/khan/CLAUDE.md"
    fi
    
    # Create GEMINI.md for Gemini CLI (same content as CLAUDE.md)
    # Gemini CLI looks for GEMINI.md in the current working directory
    ln -sf "${USER_HOME}/CLAUDE.md" "${USER_HOME}/GEMINI.md"
    chown -h "${RUNTIME_UID}:${RUNTIME_GID}" "${USER_HOME}/GEMINI.md"
    if [ -d "${USER_HOME}/khan" ]; then
        ln -sf "${USER_HOME}/CLAUDE.md" "${USER_HOME}/khan/GEMINI.md"
        chown -h "${RUNTIME_UID}:${RUNTIME_GID}" "${USER_HOME}/khan/GEMINI.md"
    fi
    
    log_success "AI agent rules installed: ~/CLAUDE.md and ~/GEMINI.md (symlinked to ~/khan/)"
...
# Check API key based on LLM_PROVIDER
# The Claude CLI requires ANTHROPIC_API_KEY to be set even when routing to other providers
...
case "$LLM_PROVIDER" in
    google|gemini)
        if [ -n "${GOOGLE_API_KEY:-}" ]; then
            ...
            log_info "  Set via: export GOOGLE_API_KEY=AIza..."
        fi
        ;;
    openai)
        if [ -n "${OPENAI_API_KEY:-}" ]; then
            ...
            log_info "  Set via: export OPENAI_API_KEY=sk-..."
        fi
        ;;
    anthropic|*)
...
# Set up claude-code-router config directory
# Router enables multi-provider support (Anthropic, OpenAI, Gemini)
mkdir -p "${USER_HOME}/.claude-code-router/plugins"
mkdir -p "${USER_HOME}/.claude-code-router/logs"
chown -R "${RUNTIME_UID}:${RUNTIME_GID}" "${USER_HOME}/.claude-code-router"
ROUTER_CONFIG="${USER_HOME}/.claude-code-router/config.json"

# Create router config based on LLM_PROVIDER preference
# LLM_PROVIDER can be: anthropic, openai, or google
LLM_PROVIDER="${LLM_PROVIDER:-anthropic}"
...
case "$LLM_PROVIDER" in
    google|gemini)
        if [ -z "${GOOGLE_API_KEY:-}" ]; then
            log_warn "LLM_PROVIDER=google but GOOGLE_API_KEY not set"
        fi
        # Google Gemini 3 Pro - most intelligent multimodal model
        # https://ai.google.dev/gemini-api/docs/models#gemini-3-pro
        # NON_INTERACTIVE_MODE required for Docker containers
        # log: true to debug routing issues
        cat > "$ROUTER_CONFIG" << 'ROUTER_CFG'
{
  "log": true,
  "LOG_LEVEL": "debug",
...
ROUTER_CFG
        log_success "Router configured for Google Gemini 3 Pro"
        log_info "  Router logs: ~/.claude-code-router/logs/ and ~/.claude-code-router/claude-code-router.log"
        ;;
    openai)
        if [ -z "${OPENAI_API_KEY:-}" ]; then
            log_warn "LLM_PROVIDER=openai but OPENAI_API_KEY not set"
        fi
        # OpenAI GPT-5.2
...
ROUTER_CFG
        log_success "Router configured for OpenAI GPT-5.2"
        ;;
    anthropic|*)
        if [ -z "${ANTHROPIC_API_KEY:-}" ]; then
            log_warn "LLM_PROVIDER=anthropic but ANTHROPIC_API_KEY not set"
        fi
        # Anthropic Claude Opus 4.5 - most intelligent model
...
ROUTER_CFG
        log_success "Router configured for Anthropic Claude Opus 4.5"
        ;;
esac
...
GEMINI_ENV="${GEMINI_SETTINGS_DIR}/.env"

mkdir -p "$GEMINI_SETTINGS_DIR"
chown "${RUNTIME_UID}:${RUNTIME_GID}" "$GEMINI_SETTINGS_DIR"

# Configure Gemini API key in .env file so it's auto-used without prompting
if [ -n "${GOOGLE_API_KEY:-}" ]; then
    # Write API key to .gemini/.env for automatic auth
    cat > "$GEMINI_ENV" << GEMINI_ENV_FILE
# Auto-configured by jib entrypoint
GEMINI_API_KEY=${GOOGLE_API_KEY}
GEMINI_ENV_FILE
    chown "${RUNTIME_UID}:${RUNTIME_GID}" "$GEMINI_ENV"
    chmod 600 "$GEMINI_ENV"
    log_success "Gemini API key configured in ~/.gemini/.env"
fi
...
if os.path.exists(settings_path) and os.path.getsize(settings_path) > 2:
...
config['autoAccept'] = True  # Auto-accept safe tool calls
...
with open(settings_path, 'w') as f:
    json.dump(config, f, indent=2)
GEMINI_BASE_CONFIG
chown "${RUNTIME_UID}:${RUNTIME_GID}" "$GEMINI_SETTINGS"
chmod 600 "$GEMINI_SETTINGS"
log_success "Gemini CLI settings configured"

# Create alias for Claude with permissions bypassed (safe in this sandboxed environment)
# Note: PATH is already set in entrypoint to include /opt/jib-runtime/jib-container/bin
...
#
# Analysis scripts location: /opt/jib-runtime/jib-container/jib-tasks/ (copied during build)
...
log_info ""
log_info "📊 Analysis Pattern: Exec-based (triggered by host services)"
...
cleanup_on_exit() {
    ...
    [ "$JIB_QUIET" = "0" ] && echo ""
    [ "$JIB_QUIET" = "0" ] && echo "🧹 Cleaning up on container exit..."
    ...
    if [ -d "${USER_HOME}/.git-main" ]; then
        for repo_config in "${USER_HOME}/.git-main"/*/config; do
            if [ -f "$repo_config" ]; then
                remote_url=$(git config -f "$repo_config" remote.origin.url 2>/dev/null || echo "")
                if [[ "$remote_url" =~ x-access-token:|ghs_|ghp_|github_pat_ ]]; then
                    echo "⚠ WARNING: Git remote URL was modified during session!"
    ...
    [ "$JIB_QUIET" = "0" ] && echo "✓ Cleanup complete"
}
...
if [ $# -eq 0 ]; then
    ...
    if [ -f "$ROUTER_CONFIG" ]; then
        log_info "Router config: $ROUTER_CONFIG"
        cat "$ROUTER_CONFIG"
        log_info ""
        
        # Start the router in background
        log_info "Starting claude-code-router..."
        gosu "${RUNTIME_UID}:${RUNTIME_GID}" ccr start --port 3456 \
            > /tmp/claude-code-router.log 2>&1 &
        ROUTER_PID=$!
        
        # Wait for router to be ready
        for _ in $(seq 1 20); do
            if nc -z localhost 3456 2>/dev/null; then
                log_success "Router started (PID: $ROUTER_PID, port: 3456)"
                break
            fi
            sleep 0.5
        done
        ...
        if ! nc -z localhost 3456 2>/dev/null; then
            log_warn "Router may not be ready - check /tmp/claude-code-router.log"
            cat /tmp/claude-code-router.log 2>/dev/null || true
        fi
        
        # Set up environment variables using ccr activate
        # This works with both 'claude' command and Agent SDK
        log_info "Activating router environment..."
        
        # Get the activate output and export each variable
        ACTIVATE_OUTPUT=$(gosu "${RUNTIME_UID}:${RUNTIME_GID}" ccr activate 2>/dev/null) || true
        log_info "ccr activate output:"
        echo "$ACTIVATE_OUTPUT"
        
        # Evaluate and export
        eval "$ACTIVATE_OUTPUT"
        export ANTHROPIC_BASE_URL ANTHROPIC_AUTH_TOKEN NO_PROXY DISABLE_TELEMETRY DISABLE_COST_WARNINGS API_TIMEOUT_MS 2>/dev/null || true
        
        log_info ""
        log_info "Environment after activation:"
        log_info "  ANTHROPIC_BASE_URL=${ANTHROPIC_BASE_URL:-not set}"
        log_info "  ANTHROPIC_AUTH_TOKEN=${ANTHROPIC_AUTH_TOKEN:+set}"
    fi
    log_info ""
    ...
    if [ "$JIB_QUIET" = "0" ]; then
        ...
        echo "  • ~/CLAUDE.md                      (concise rules: mission, beads, context, etc.)"
        echo "  • ~/khan/james-in-a-box/docs/      (reference docs - fetch on demand)"
        ...
        echo ""
        echo "💡 Tips:"
        echo "  • ~/khan/ is READ-WRITE - make changes directly, commit to git"
        echo "  • Human reviews commits - you can push directly via git push"
        ...
        echo "  • GitHub App token for git push (HTTPS only, no SSH keys)"
        echo "  • No cloud credentials (cannot deploy to GCP/AWS)"
        echo ""
        echo "======================================================================"
        echo ""
    else
    ...
    # Launch interactive mode via unified llm module
    # The llm.runner.run_interactive() handles provider selection and configuration
    log_info "Launching LLM interactive mode via llm module (provider: ${LLM_PROVIDER:-anthropic})..."
    exec gosu "${RUNTIME_UID}:${RUNTIME_GID}" env \
        ...
        python3 -c "from llm import run_interactive; run_interactive()"
else
    # Non-interactive mode (--exec)
    case "$LLM_PROVIDER" in
        google|gemini)
            # Gemini native mode - no router needed
            log_info "Exec mode with Gemini CLI environment"
            exec gosu "${RUNTIME_UID}:${RUNTIME_GID}" env \
                GOOGLE_API_KEY="${GOOGLE_API_KEY:-}" \
                GEMINI_API_KEY="${GOOGLE_API_KEY:-}" \
                "$@"
            ;;
        *)
            # Claude/OpenAI - start router for claude-agent-sdk
            ROUTER_CONFIG="${USER_HOME}/.claude-code-router/config.json"
            if [ -f "$ROUTER_CONFIG" ]; then
                log_info "Starting claude-code-router for exec mode..."
                # Use ccr activate to set up environment variables
                eval "$(gosu "${RUNTIME_UID}:${RUNTIME_GID}" ccr activate 2>/dev/null)" || true
                ...
                # Wait for router (up to 5 seconds for exec mode)
                for _ in $(seq 1 10); do
                    if kill -0 $ROUTER_PID 2>/dev/null && nc -z localhost 3456 2>/dev/null; then
                        log_success "Router started (PID: $ROUTER_PID)"
                        break
                    fi
                    sleep 0.5
                done
