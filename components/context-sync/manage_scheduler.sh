...
update_timer_frequency() {
    ...
    local timer_source="${SCRIPT_DIR}/systemd/context-sync.timer"
...
case "${1:-}" in
    enable)
        ...
        ln -s "${SCRIPT_DIR}/systemd/context-sync.service" ~/.config/systemd/user/
        ln -s "${SCRIPT_DIR}/systemd/context-sync.timer" ~/.config/systemd/user/
